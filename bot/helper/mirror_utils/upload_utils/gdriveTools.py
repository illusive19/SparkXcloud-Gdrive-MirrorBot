...
class GoogleDriveHelper:
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            'name': file_name,
            'description': "👨‍🦱 Uᴘʟᴏᴀᴅᴇᴅ Usɪɴɢ SᴘᴀʀᴋxCʟᴏᴜᴅ-Gᴅʀɪᴠᴇ-Mɪʀʀᴏʀʙᴏᴛ",
            'mimeType': mime_type,
        }
    ...
    def deletefile(self, link: str):
        try:
            ...
            msg = "📛 Gᴏᴏɢʟᴇ Dʀɪᴠᴇ Iᴅ Cᴏᴜʟᴅ Nᴏᴛ Bᴇ Fᴏᴜɴᴅ Iɴ Tʜᴇ Pʀᴏᴠɪᴅᴇᴅ Lɪɴᴋ"
            return msg
        msg = ''
        try:
            res = self.__service.files().delete(fileId=file_id, supportsTeamDrives=IS_TEAM_DRIVE).execute()
            msg = "Sᴜᴄᴄᴇssғᴜʟʟʏ Dᴇʟᴇᴛᴇᴅ"
        except HttpError as err:
            LOGGER.error(str(err))
            if "File not found" in str(err):
                msg = "Nᴏ Sᴜᴄʜ Fɪʟᴇ Exɪsᴛ"
            else:
                msg = "Sᴏᴍᴇᴛʜɪɴɢ Wᴇɴᴛ Wʀᴏɴɢ Cʜᴇᴄᴋ Lᴏɢ"
        finally:
            return msg
    ...
    def upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            'name': file_name,
            'description': "💮 Uᴘʟᴏᴀᴅᴇᴅ Bʏ SᴘᴀʀᴋxCʟᴏᴜᴅ-Gᴅʀɪᴠᴇ-Mɪʀʀᴏʀʙᴏᴛ",
            'mimeType': mime_type,
        }
        try:
    ...
    def clone(self, link):
        ...
        try:
            ...
            if "User rate limit exceeded" in str(err):
                msg = "😑 Usᴇʀ Rᴀᴛᴇ Lɪᴍɪᴛ Exᴄᴇᴇᴅᴇᴅ, Oɴᴄᴇ Cʜᴇᴄᴋ Uʀ Lɪɴᴋ Fɪʀsᴛ."
            elif "File not found" in str(err):
                msg = "☹️ Fɪʟᴇ Nᴏᴛ Fᴏᴜɴᴅ."
            else:
                msg = f"Error.\n{err}"
            return msg, ""
    ...
    def edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            ...
            Telegraph(access_token=telegraph_token).edit_page(path = self.path[prev_page],
                                 title = '💞 SᴘᴀʀᴋxCʟᴏᴜᴅ-Gᴅʀɪᴠᴇ-Mɪʀʀᴏʀʙᴏᴛ Sᴇᴀʀᴄʜ',
                                 author_name='💓 SᴘᴀʀᴋxCʟᴏᴜᴅ-Gᴅʀɪᴠᴇ-Mɪʀʀᴏʀʙᴏᴛ',
    ...
    def drive_list(self, fileName):
        ...
        content_count = 0
        if response["files"]:
            msg += f'<h4>{len(response["files"])} ✅ Sᴇᴀʀᴄʜ Rᴇsᴜʟᴛ Fᴏʀ: {fileName}</h4><br><br>'
            for file in response.get('files', []):
            ...
            msg = f"<b>✅ Fᴏᴜɴᴅ<code>{len(response['files'])}</code> results for <code>{fileName}</code></b>"
            buttons = button_build.ButtonMaker()   
